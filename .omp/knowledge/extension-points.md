...
| **新增订阅 provider** | 插件 manifest 加 `subscriptions:[{providerId,entry,timeoutMs}]`，脚本实现 `globalThis.subscribe(ctx)` 并返回规范化条目 → `PluginManager` 动态路由到 `subscription::SubscriptionProvider` → 公共调度负责退避/去重/过滤/落库/建任务；纯 Rust 宿主也可直接调用 `DownloadManager::register_subscription_provider` |
